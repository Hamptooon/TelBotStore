...
def get_total_coast(DB, message):
    all_products_id = DB.select_all_products_id(message.from_user.id)
    all_price = [DB.select_single_product_price(itm) for itm in all_products_id]
    all_quantity = [DB.select_order_quantity(itm, message.from_user.id) for itm in all_products_id]
